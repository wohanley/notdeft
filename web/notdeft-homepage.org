...
#+MTIME: Tue, 26 Jun 2018 18:18:18 +0300
...
Download the source code into some directory with
...
To have NotDeft use the =notdeft-xapian= program you've built, you will have to specify its absolute path in the =notdeft-xapian-program= variable. You could use =M-x customize-variable= to set it, or simply
...
To enter a query, press =TAB= first. To clear a query, you can
...
If enabling =notdeft-note-mode= directly in “.dir-locals.el” does not work or appeal to you, then it's possible to do the same thing indirectly, by using an actual per-directory local variable to indicate if the minor mode should be enabled:
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook
   'org-mode-local-vars-hook
   (lambda ()
     (when notdeft-note-mode-auto-enable
       (notdeft-note-mode 1))))
#+END_SRC
This does not work if done in =org-mode-hook=, as directory locals are not set yet, and thus what is needed is a later hook. Borrowing code from “phils” at Stack Overflow, we can get our =org-mode-local-vars-hook= run by defining a new kind of hook as
#+BEGIN_SRC emacs-lisp
  (defun run-local-vars-mode-hooks ()
  ...
  directory local variables set."
    (run-hooks (intern (concat (symbol-name major-mode)
			       "-local-vars-hook"))))
  (add-hook 'hack-local-variables-hook 'run-local-vars-mode-hooks)
#+END_SRC

To set the =notdeft-note-mode-auto-enable= flag for the entire directory of a note file that you have opened, you may use the Emacs command
: M-x add-dir-local-variable RET notdeft-note-mode-auto-enable RET t RET
Of course, if desired, you could also use =add-file-local-variable= to instruct the mode to be enabled for just the file.
...
#+BEGIN_SRC emacs-lisp
  (let ((org-contacts-files
	 (notdeft-list-files-by-query
	  "!all ext:Org AND Email")))
    (org-contacts name))
#+END_SRC

Similarly, we might use =org-agenda='s =org-todo-list= command to list to-do entries, but resolving the =org-agenda-files= list on demand by looking for the “TODO” keyword in any Org files in our collection:
#+BEGIN_SRC emacs-lisp
...
For example, if you've highlighted the text “desktop wikis”, the command will offer to create a note of the same title, derive a filename for it based on the title, and replace the region with a “deft:” link to it. (The command is defined by the =notdeft-org9= feature.)
...
6. If you see unexpected behavior after setting a search query, ensure that the =notdeft-xapian-program= variable names a working executable. It may be worth trying to run the program directly, and seeing what it says. For example:    
