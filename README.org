...
** Note File Format
   ...
   The default is to have the note filename =deft-extension= set to "org", and its =deft-text-mode= set to ='org-mode=. If you use some other note format, you should change those settings, which can be done with
   : M-x customize-variable RET deft-extension RET
   : M-x customize-variable RET deft-text-mode RET
...
** Query Modifiers
   ...
   - =!max= :: Prefix a query with =!max= to show all matching results.
