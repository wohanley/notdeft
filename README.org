...
** Prefixes
   ...
   - =ext:= :: Indicates the exact string that must be the lowercased filename extension of the file (without the ".").
...
* Example Search Queries
  ...
  Filename extensions must be given in lowercase, and without allowing stemming. For example, to find all Org documents that may contain open to-do entries, we might query with:
