...
/NotDeft/ is an experimental spin-off of [[http://jblevins.org/projects/deft/][Deft]], an “Emacs mode for quickly browsing, filtering, and editing directories of plain text notes.” NotDeft is less configurable than mainline [[https://github.com/jrblevin/deft][Deft]], but it features a Xapian backend for efficient free-text search over potentially very large numbers of note files; in that respect it is like the [[http://notmuchmail.org/][Notmuch]] Emacs mode for managing email.

The key differences of NotDeft are:
  1. Rather than supporting a single, customizable =deft-directory= (tree) of note files, it supports a customizable =notdeft-directories= search path of directory trees.
...
The added complication of having to manage multiple directories and having two stages to the process of looking for interesting files makes NotDeft less simple and intuitive than Deft. NotDeft is not Deft---it is not a Notational Velocity type application, but rather it aims to be a local search engine, one that is also accessible by other Emacs-based applications.
...
* Using NotDeft from Outside NotDeft Mode
